    ...
    if magicOK:
        ...
        for tlvIdx in range(numTLVs):
            ...
            if tlv_type == MMWDEMO_UART_MSG_DETECTED_POINTS:
                ...
                rngObj = processRangeNoiseProfile(byteBuffer, idX, detObj, configParameters, True)
                finalobj.update(rngObj)
            elif tlv_type == MMWDEMO_OUTPUT_MSG_NOISE_PROFILE:
                noiseObj = processRangeNoiseProfile(byteBuffer, idX, detObj, configParameters, False)
                ...
                pass
                # continue
                rangeDoppler = np.reshape(rangeDoppler, (configParameters["numDopplerBins"],
                                                         configParameters["numRangeBins"]), 'F')
                rangeDoppler = np.append(rangeDoppler[int(len(rangeDoppler) / 2):],
                                         rangeDoppler[:int(len(rangeDoppler) / 2)], axis=0)
                rangeArray = np.array(range(configParameters["numRangeBins"])) * configParameters["rangeIdxToMeters"]
                dopplerArray = np.multiply(
                ...
                doppObj = {'rangeDoppler': list(rangeDoppler), 'rangeArray': list(rangeArray),
